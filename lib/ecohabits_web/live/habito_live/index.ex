defmodule EcohabitsWeb.HabitoLive.Index do
  ...
  def handle_event("save", %{"habito" => habit_params}, socket) do
    # Temporário: Simulando um usuário logado (Módulo A) e pontuação padrão
    habit_params = 
      habit_params
      |> Map.put("usuario_id", 1)
      |> Map.update("pontuacao", "0", fn p -> if p == "", do: "0", else: p end)

    case Habitos.create_habito(habit_params) do
      {:ok, _habit} ->
        {:noreply,
         socket
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      <div class="space-y-6">
        ...
        <div :if={@live_action == :new} class="modal modal-open" id="habit-modal">
          <div class="modal-box bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 border-none relative">
            <h2 class="text-2xl text-gray-800 mb-6">Novo Hábito Sustentável</h2>
            <.form for={@form} phx-submit="save" class="space-y-4">
              ...
              <div>
                ...
                <.input field={@form[:nome]} type="text" placeholder="Ex: Separar lixo reciclável" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>

              <div>
                <label class="block text-sm text-gray-700 mb-2">Descrição</label>
                <.input field={@form[:descricao]} type="textarea" placeholder="Descreva o hábito..." rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>

              <div>
                <label class="block text-sm text-gray-700 mb-2">Categoria</label>
                <.input field={@form[:categoria_id]} type="select" options={[{"Alimentação", 1}, {"Transporte", 2}, {"Energia", 3}, {"Água", 4}, {"Resíduos", 5}, {"Outros", 6}]} class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>

              <div>
                <label class="block text-sm text-gray-700 mb-2">Pontuação</label>
                <.input field={@form[:pontuacao]} type="number" placeholder="10" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              ...
              <div class="flex gap-3 mt-6">
                ...
                <button type="submit" class="flex-1 px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-lg hover:from-emerald-600 hover:to-teal-700 transition-all shadow-md">
                  Criar Hábito
                </button>
              </div>
            </.form>
